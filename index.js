...
// ── Check if ninja already exists with the correct version ──
function checkExistingNinja(destDir, expectedVersion) {
    const ninjaName = process.platform === 'win32' ? 'ninja.exe' : 'ninja'
    const fullPath = path.join(destDir, ninjaName)

    if (!fs.existsSync(fullPath)) return false

    const result = spawn(fullPath, ['--version'], { encoding: 'utf8' })
    if (result.error) return false

    const installedVersion = result.stdout.trim()
    if (installedVersion === expectedVersion) {
        console.log(`ninja ${installedVersion} already exists at '${fullPath}', skipping download`)
        return true
    }

    console.log(`existing ninja version ${installedVersion} does not match required ${expectedVersion}, re-downloading`)
    return false
}
...
async function run() {
    ...
    // ── Skip download if ninja already exists with correct version ──
    if (checkExistingNinja(fullDestDir, version)) {
